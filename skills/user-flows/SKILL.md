...
Gitignored files are skipped automatically, including via nested `.gitignore` files.
Vendored trees that git *does* track, such as third-party templates or bundled
example apps, need `--exclude 'vendor/,templates/'` in gitignore syntax. If the
included list is dominated by code that is not this project's own surface, that is
what `--exclude` is for. Do not lower the threshold to compensate.
